...
#
# --pages needs the gh cli, logged in or carrying GH_TOKEN, plus jq.
...
"
  ...
  # the store listing is not a release asset, so it is spelled out here
...
do_pages=0 do_crates=0 tag="" cargo_args=()

while (($#)); do
  case "$1" in
    --all) do_pages=1 do_crates=1 ;;
    --pages) do_pages=1 ;;
    --crates) do_crates=1 ;;
    --dry-run) cargo_args+=(--dry-run) ;;
    -h | --help) usage; exit 0 ;;
...
((do_pages)) && pages "$tag"
((do_crates)) && crates "${cargo_args[@]+"${cargo_args[@]}"}"
exit 0
